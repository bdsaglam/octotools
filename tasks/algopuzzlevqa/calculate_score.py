...
class ResultScorer:
    ...
    def normalize_answer(answer):
        ...
        normalized = normalized.strip(
            ".()'\""
        )  # Remove surrounding punctuation or quotes
        return answer_mapping.get(
            normalized, normalized
        )  # Map to standard form or return as is
    ...
    def answer_verification(self, response, correct_answer):
        ...
        try:
            is_correct = normalized_pred == normalized_gt or normalized_pred.startswith(
                normalized_gt
            )
    ...
    def score_results(self, results, max_workers=10):
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in tqdm.tqdm(
                ...
                results[pid].update(
                    {"stepwise_analysis": analysis, "true_false": true_false}
                )
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Extract and score the results from the benchmark data"
    )
    parser.add_argument(
        "--data_file",
        type=str,
        default="data/val.json",
        help="The file containing the benchmark data",
    )
    parser.add_argument(
        "--result_dir",
        type=str,
    ...
    parser.add_argument(
        "--log_dir", type=str, default=None, help="The directory containing the logs"
    )
    parser.add_argument(
        "--response_type",
        type=str,
...
if __name__ == "__main__":
    ...
    if os.path.exists(log_dir):
        ...
        # Update the scores
        scores.update({"step_stats": step_stats, "tool_usage": tool_usage})

    # Save the scores
    score_file = os.path.join(
        args.result_dir, f"final_scores_{args.response_type}.json"
    )
